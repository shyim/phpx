...
static char *phpx_script_filename = NULL;
static char *phpx_ini_entries = NULL;

static void phpx_register_variables(zval *track_vars_array) {
    /* Import environment variables */
    php_import_environment_variables(track_vars_array);

    if (phpx_script_filename != NULL) {
        size_t len = strlen(phpx_script_filename);
        php_register_variable_safe("PHP_SELF", phpx_script_filename, len, track_vars_array);
        php_register_variable_safe("SCRIPT_NAME", phpx_script_filename, len, track_vars_array);
        php_register_variable_safe("SCRIPT_FILENAME", phpx_script_filename, len, track_vars_array);
        php_register_variable_safe("PATH_TRANSLATED", phpx_script_filename, len, track_vars_array);
    }
...
static void phpx_register_file_handles(void) {
...
void phpx_set_ini_entries(const char *entries) {
    if (phpx_ini_entries != NULL) {
        free(phpx_ini_entries);
    }
    if (entries != NULL) {
        phpx_ini_entries = strdup(entries);
    } else {
        phpx_ini_entries = NULL;
    }
}

/* Parse and apply ini entries after PHP startup */
static void phpx_apply_ini_entries(void) {
    if (phpx_ini_entries == NULL) {
        return;
    }

    char *entries = strdup(phpx_ini_entries);
...
static int phpx_init(int argc, char **argv) {
    ...
    phpx_register_file_handles();

    /* Apply INI entries after startup */
    phpx_apply_ini_entries();
...
int phpx_execute_script(const char *script_path, int argc, char **argv) {
    ...
    phpx_script_filename = (char *)script_path;

    if (phpx_init(argc, argv) != 0) {
        return 1;
    }
...
int phpx_execute_code(const char *code, int argc, char **argv) {
    ...
    phpx_script_filename = "Command line code";
 ...
 */
int phpx_lint_file(const char *script_path, int argc, char **argv) {
    int result = 0;
 ...
 */
int phpx_print_modules(int argc, char **argv) {
    phpx_script_filename = "modules";
 ...
 */
char *phpx_get_loaded_extensions(int argc, char **argv) {
    phpx_script_filename = "extensions";
 ...
 */
void phpx_free_string(char *str) {
    if (str != NULL) {
        free(str);
    }
}
...
static __thread phpx_request_context *current_request = NULL;
...
static size_t phpx_web_ub_write(const char *str, size_t str_length) {
    append_response_body(str, str_length);
    return str_length;
}

/* SAPI: Flush output */
static void phpx_web_sapi_flush(void *server_context) {
    /* We buffer everything, so flush is a no-op */
    (void)server_context;
}

/* SAPI: Send headers */
static int phpx_web_send_headers(sapi_headers_struct *sapi_headers) {
...
/* SAPI: Read POST data */
static size_t phpx_web_read_post(char *buffer, size_t count_bytes) {
...
/* SAPI: Read cookies */
static char *phpx_web_read_cookies(void) {
...
static void phpx_web_register_variables(zval *track_vars_array) {
    if (current_request == NULL) return;
...
static int phpx_web_startup(sapi_module_struct *sapi_module) {
    return php_module_startup(sapi_module, NULL);
}

/* Custom SAPI module for web requests */
static sapi_module_struct phpx_web_sapi_module = {
    "phpx",                         /* name */
    "phpx Web Server",              /* pretty name */

    phpx_web_startup,               /* startup */
    php_module_shutdown_wrapper,    /* shutdown */

    NULL,                           /* activate */
    NULL,                           /* deactivate */

    phpx_web_ub_write,              /* unbuffered write */
    phpx_web_sapi_flush,            /* flush */
    NULL,                           /* get uid */
    NULL,                           /* getenv */
    ...
    NULL,                           /* header handler */
    phpx_web_send_headers,          /* send headers handler */
    NULL,                           /* send header handler */

    phpx_web_read_post,             /* read POST data */
    phpx_web_read_cookies,          /* read Cookies */

    phpx_web_register_variables,    /* register server variables */
    NULL,                           /* Log message */
...
static int phpx_web_initialized = 0;

/*
 * Initialize the web SAPI (call once at server startup).
 */
int phpx_web_init(void) {
    if (phpx_web_initialized) {
        return 0;
    }
    ...
    sapi_startup(&phpx_web_sapi_module);

    phpx_web_sapi_module.ini_entries = phpx_ini_entries;

    if (phpx_web_sapi_module.startup(&phpx_web_sapi_module) == FAILURE) {
        return 1;
    }

    phpx_web_initialized = 1;
    return 0;
}
 ...
 */
int phpx_web_execute(phpx_request_context *ctx) {
    if (!phpx_web_initialized) {
        if (phpx_web_init() != 0) {
            return 1;
        }
 ...
 */
size_t phpx_request_context_size(void) {
    return sizeof(phpx_request_context);
}

/*
 * Free response buffers in the request context.
 */
void phpx_free_response(phpx_request_context *ctx) {
...
typedef struct {
    ...
    phpx_request_context *pending_request;  /* The pending request to handle */
} phpx_worker_state;

static __thread phpx_worker_state worker_state = {0};

/* Callback from Rust when a new request is available */
extern int phpx_worker_wait_for_request(void);
extern void phpx_worker_request_done(void);

/*
 * PHP function: phpx_handle_request(callable $callback): bool
 *
 ...
 */
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_phpx_handle_request, 0, 1, _IS_BOOL, 0)
    ZEND_ARG_TYPE_INFO(0, callback, IS_CALLABLE, 0)
ZEND_END_ARG_INFO()

PHP_FUNCTION(phpx_handle_request) {
    zend_fcall_info fci;
    ...
    if (!worker_state.is_worker_mode) {
        zend_throw_exception(
            ...
            "phpx_handle_request() called while not in worker mode", 0);
        RETURN_THROWS();
    }
    ...
    int got_request = phpx_worker_wait_for_request();
...
static const zend_function_entry phpx_functions[] = {
    PHP_FE(phpx_handle_request, arginfo_phpx_handle_request)
    PHP_FE_END
};

static zend_module_entry phpx_module_entry = {
...
static int phpx_worker_startup(sapi_module_struct *sapi_module) {
    if (php_module_startup(sapi_module, &phpx_module_entry) == FAILURE) {
        return FAILURE;
    }
    return SUCCESS;
}
...
static sapi_module_struct phpx_worker_sapi_module = {
    "phpx-worker",                  /* name */
    "phpx Worker Mode",             /* pretty name */

    phpx_worker_startup,            /* startup - register our extension */
    php_module_shutdown_wrapper,    /* shutdown */
...
static int phpx_worker_global_initialized = 0;

/*
 * Global initialization for worker mode (call once from main thread before spawning workers).
 */
int phpx_worker_global_init(void) {
    if (phpx_worker_global_initialized) {
        return 0;
    }
    ...
    sapi_startup(&phpx_worker_sapi_module);

    phpx_worker_sapi_module.ini_entries = phpx_ini_entries;

    if (phpx_worker_sapi_module.startup(&phpx_worker_sapi_module) == FAILURE) {
        return 1;
    }

    phpx_worker_global_initialized = 1;
    return 0;
}

/*
 * Initialize a worker thread (call once per worker thread).
 * Must be called from each worker thread after phpx_worker_global_init()
 * has been called from the main thread.
 */
int phpx_worker_init(const char *script_filename, const char *document_root) {
 ...
 */
void phpx_worker_set_request(phpx_request_context *ctx) {
 ...
 */
int phpx_worker_is_waiting(void) {
 ...
 */
int phpx_worker_has_response(void) {
    return 0; /* Response state is tracked in Rust */
}

/*
 * Execute the worker script. This runs the worker script which should
 * contain a loop calling phpx_handle_request().
 *
 * IMPORTANT: phpx_worker_global_init() must be called from the main thread
 * before calling this function from worker threads.
 */
int phpx_worker_run(const char *script_filename, const char *document_root) {
    /* Per-thread initialization */
    phpx_worker_init(script_filename, document_root);

    /* Create a dummy request context for the initial script execution */
    phpx_request_context dummy_ctx = {0};
 ...
 */
void phpx_worker_shutdown(void) {
    worker_state.is_worker_mode = 0;
}
