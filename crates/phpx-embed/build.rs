...
fn main() {
    ...
    let static_linking = env::var("PHPX_STATIC").is_ok()
    ...
    if static_linking {
        ...
        if target_os == "macos" {
            // Note: iconv is handled via link-arg in phpx's build.rs to ensure
        ...
        // Note: rpath is set in the phpx crate's build.rs, not here
    ...
    println!("cargo:rerun-if-env-changed=PHPX_STATIC");
}
